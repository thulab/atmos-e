...
B_IP_list=(0 172.20.31.41 172.20.31.42 172.20.31.43)
config_schema_replication_factor=(0 1 1 1)
config_data_replication_factor=(0 1 1 1)
#config_schema_replication_factor=(0 3 3 3)
#config_data_replication_factor=(0 3 3 3)
...
START_TIME=(0 2023-01-02T00:00:00+08:00 2023-01-01T00:00:00+08:00 2023-01-01T00:00:00+08:00)
...
DBNAME="QA_ATM"  #数据库名称
TABLENAME="test_result_cluster_insert_dt" #数据库中表的名称
...
if [ "$check_config_num" == "$config_num" ] && [ "$check_data_num" == "$data_num" ]; then
	...
	then
		for ((j = 1; j <= $bm_num; j++)); do
			...
			ssh ${ACCOUNT}@${B_IP_list[${j}]} "sed -i \"s/^START_TIME.*$/START_TIME=${START_TIME[${j}]}/g\" ${BM_PATH}/conf/config.properties"
	...
	while true; do
		...
		do
			...
			if [ "$dn_pid" = "" ] && [ "${cn_pid}" = "" ]; then
				...
				temp_thread_num_d=$(ssh ${ACCOUNT}@${D_IP_list[${j}]} "pstree -p \$(ps aux | grep -v grep | grep DataNode | awk '{print \$2}'" 2>/dev/null)

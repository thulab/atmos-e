...
parse_node_ingestion_result() {
    ...
    result_line="$(read_benchmark_csv_operation_result "${csv_file}" "INGESTION")" || return 1
    IFS=$'\t' read -r \
        ok_operation ok_point fail_operation fail_point throughput_value \
        latency_value min_value p10 p25 median p75 p90 p95 p99 p999 max_value <<< "${result_line}"
...
collect_node_result() {
    ...
    [ -n "${csv_file}" ] || return 0
    parse_node_ingestion_result "${csv_file}" "${index}" || return 1
}

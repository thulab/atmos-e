...
IP_list=(0 172.20.31.7 172.20.31.105)
PIPE_list=(0 172.20.31.105 172.20.31.7)
Control=172.20.31.51
config_node_config_nodes=(0 172.20.31.7:10710 172.20.31.105:10710)
data_node_config_nodes=(0 172.20.31.7:10710 172.20.31.105:10710)
############mysql信息##########################
...
DBNAME="QA_ATM"  #数据库名称
TABLENAME="test_result_pipe_test_win" #数据库中表的名称
...
setup_env() {
	...
	if [ "${ts_type}" = "tablemode" ]; then
		...
		do
			TEST_IP=${IP_list[$i]}
			str1=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect table -h ${TEST_IP} -p 6667 -u root -pw root -e "create pipe test with source ('source.realtime.mode'='stream','source.realtime.enable'='true','source.forwarding-pipe-requests'='false','source.batch.enable'='true','source.history.enable'='true') with sink ('sink'='iotdb-thrift-sink', 'sink.node-urls'='${PIPE_list[$i]}:6667');")
			str1=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect table -h ${TEST_IP} -p 6667 -u root -pw root -e "start pipe test;")
			str1=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect table -h ${TEST_IP} -p 6667 -u root -pw root -e "show pipes;" | grep 'Total line number = 1')
			#echo $str1
		...
		do
			TEST_IP=${IP_list[$i]}
			str1=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect tree -h ${TEST_IP} -p 6667 -u root -pw root -e "create pipe test with source ('source.realtime.mode'='stream','source.realtime.enable'='true','source.forwarding-pipe-requests'='false','source.batch.enable'='true','source.history.enable'='true') with sink ('sink'='iotdb-thrift-sink', 'sink.node-urls'='${PIPE_list[$i]}:6667');")
			str1=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect tree -h ${TEST_IP} -p 6667 -u root -pw root -e "start pipe test;")
			str1=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect tree -h ${TEST_IP} -p 6667 -u root -pw root -e "show pipes;" | grep 'Total line number = 1')
			echo $str1
			if [ "$str1" = "Total line number = 1" ]; then
				echo "PIPE is ready"
				pipeflag=$[${pipeflag}+1]
...
monitor_test_status() { # 监控测试运行状态，获取最大打开文件数量和最大线程数
	...
	while true; do
		...
		if [ $flagB -ge 2 ]; then
			if [ "${ts_type}" = "tablemode" ]; then
				fstr1=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect table -h ${IP_list[1]} -p 6667 -u root -pw root -e "flush;")
				fstr2=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect table -h ${IP_list[2]} -p 6667 -u root -pw root -e "flush;")
			else
				fstr1=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect tree -h ${IP_list[1]} -p 6667 -u root -pw root -e "flush;")
				fstr2=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect tree -h ${IP_list[2]} -p 6667 -u root -pw root -e "flush;")
			fi
			#BM写入结束前不进行判定
			#确认是否测试已结束
			flag=0
			if [ "${ts_type}" = "tablemode" ]; then
				str0=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect table -h ${IP_list[1]} -p 6667 -u root -pw root -e "select count(s_0) from test_g_0.table_0 where device_id = 'd_0';" | grep -o '172800' | wc -l )
			else
				str0=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect tree -h ${IP_list[1]} -p 6667 -u root -pw root -e "select count(s_0) from root.test.g_0.d_0;" | grep -o '172800' | wc -l )
			fi
			#echo "str0=${str0}"
			if [ "$str0" = "1" ]; then

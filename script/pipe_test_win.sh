...
monitor_test_status() { # 监控测试运行状态，获取最大打开文件数量和最大线程数
	...
	while true; do
		...
		if [ $flagB -ge 2 ]; then
			...
			flag=0
			if [ "${ts_type}" = "tablemode" ]; then
				str0=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect table -h ${IP_list[1]} -p 6667 -e "select count(s_0) from test_g_0.table_0 where device_id = 'd_0';" | grep -o '172800' | wc -l )
			else
				str0=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect tree -h ${IP_list[1]} -p 6667 -e "select count(s_0) from root.test.g_0.d_0;" | grep -o '172800' | wc -l )
			fi
			#echo "str0=${str0}"
			if [ "$str0" = "1" ]; then
				...
				do
					if [ "${ts_type}" = "tablemode" ]; then
						str1=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect table -h ${IP_list[1]} -p 6667 -e "select count(s_0) from test_g_0.table_0 where device_id = 'd_${device}';" | grep -o '172800' | wc -l )
						str2=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect table -h ${IP_list[2]} -p 6667 -e "select count(s_0) from test_g_0.table_0 where device_id = 'd_${device}';" | grep -o '172800' | wc -l )
						...
						str1=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect tree -h ${IP_list[1]} -p 6667 -e "select count(*) from root.test.g_0.d_${device};" | grep -o '172800' | wc -l )
						str2=$(${TEST_IOTDB_PATH}/sbin/start-cli.sh -sql_dialect tree -h ${IP_list[2]} -p 6667 -e "select count(*) from root.test.g_0.d_${device};" | grep -o '172800' | wc -l )
					fi

#!/usr/bin/env bash
# 场景名称：Compaction 性能测试
# 测试目的：验证顺序、乱序和跨空间 compaction 的执行耗时、吞吐和资源消耗。
# 具体步骤：
# 步骤一：选择待测 IoTDB 版本并准备 compaction 数据集。
# 步骤二：部署 IoTDB，写入 compaction 相关配置并启动服务。
# 步骤三：拷贝数据集并等待目标 compaction 条件满足。
# 步骤四：开启 compaction，监控完成状态并解析 compaction 日志。
# 步骤五：采集前后文件和系统指标，回写数据库并备份测试现场。
...
readonly SCENARIO_ID="compaction"
readonly TASK_TEST_TYPE="compaction"
readonly SCENARIO_FAILURE_POLICY="continue_and_fail"
readonly -a SCENARIO_CASE_DIMENSIONS=("protocol=211" "type=common,aligned")
...
scenario_case_execute() { test_operation "${CASE_PROTOCOL}" "${CASE_TYPE}"; }

...
BUCKUP_PATH=/nasdata/compaction
...
SERVERTABLE="SERVER_MODE_compaction"
############公用函数##########################
...
modify_iotdb_config() { # iotdb调整内存，关闭合并
	...
	#关闭影响写入性能的其他功能
	#sed -i "s/^# enable_partition=.*$/enable_partition=false/g" ${TEST_DATANODE_PATH}/conf/iotdb-common.properties
...
monitor_test_status() { # 监控测试运行状态，获取最大打开文件数量和最大线程数
	...
	do
		...
		#监控合并执行情况  
		cd ${TEST_DATANODE_PATH}/data/datanode/data
		...
		if [ ${numOfcompactioning} -le 0 ]; then
			...
			numOfcompactioning=$(find . -name "*compaction.log" | wc -l)
			if [ ${numOfcompactioning} -le 0 ]; then
				log_compaction=${TEST_DATANODE_PATH}/logs/log_datanode_compaction.log
...
collect_data_before() { # 收集iotdb数据大小，顺、乱序文件数量
	cd ${TEST_DATANODE_PATH}
	dataFileSize_before=$(du -h -d0 ${TEST_DATANODE_PATH}/data | awk {'print $1'} | awk '{sub(/.$/,"")}1')
	...
	numOfSe0Level_before=$(find ${TEST_DATANODE_PATH}/data/datanode/data/sequence -name "*-0-*.tsfile" | wc -l)
...
collect_data_after() { # 收集iotdb数据大小，顺、乱序文件数量
	#收集启动后基础监控数据
	cd ${TEST_DATANODE_PATH}
	dataFileSize_after=$(du -h -d0 ${TEST_DATANODE_PATH}/data | awk {'print $1'} | awk '{sub(/.$/,"")}1')
	UNIT=$(du -h -d0 ${TEST_DATANODE_PATH}/data | awk {'print $1'} | awk -F '' '$0=$NF')
	if [ "$UNIT" = "M" ]; then
	...
	numOfSe0Level_after=$(find ${TEST_DATANODE_PATH}/data/datanode/data/sequence -name "*-0-*.tsfile" | wc -l)
	if [ ! -d "${TEST_DATANODE_PATH}/data/datanode/data/unsequence" ]; then
		numOfUnse0Level_after=0
	else
		#cd ${TEST_DATANODE_PATH}/data/datanode/data/unsequence
		numOfUnse0Level_after=$(find ${TEST_DATANODE_PATH}/data/datanode/data/unsequence -name "*-0-*.tsfile" | wc -l)
	fi
	compaction_rate=0
	ts_dataSize=0
	ts_numOfPoints=0
	#测试结果写入数据库
	comp_start_time=$(awk 'NR==1{print $1,$2}' ${TEST_DATANODE_PATH}/logs/log_datanode_compaction.log| cut -c 1-19)
	comp_end_time=$(awk 'END{print $1,$2}' ${TEST_DATANODE_PATH}/logs/log_datanode_compaction.log| cut -c 1-19)
	#cost_time=$(($(date +%s -d "${comp_end_time}") - $(date +%s -d "${comp_start_time}")))
	cd ${TEST_DATANODE_PATH}/logs/
...
drop_monitor_table() { # 备份测试数据
	#同步服务器监控数据到统一的表内
	copy_sql="USE ${DBNAME};REPLACE INTO ${SERVERTABLE} select * from ${SERVERTABLE}_${data1};"
	mysql -h${MYSQLHOSTNAME} -P${PORT} -u${USERNAME} -p${PASSWORD} ${DBNAME} -e "${copy_sql}"
	#清理监控临时表
	drop_sql="USE ${DBNAME};drop table if exists ${SERVERTABLE}_${data1};"
	mysql -h${MYSQLHOSTNAME} -P${PORT} -u${USERNAME} -p${PASSWORD} ${DBNAME} -e "${drop_sql}"
}
test_operation() {
	protocol_class=$1
	ts_type=$2
	...
	sed -i "s/^# enable_unseq_space_compaction=.*$/enable_unseq_space_compaction=false/g" ${TEST_DATANODE_PATH}/conf/iotdb-common.properties
	sed -i "s/^# enable_cross_space_compaction=.*$/enable_cross_space_compaction=false/g" ${TEST_DATANODE_PATH}/conf/iotdb-common.properties
	#收集启动前基础监控数据
	collect_data_before
	#启动iotdb和monitor监控
	start_iotdb
	...
	collect_data_after
	insert_database ${protocol_class}
	if [ -d "${TEST_DATANODE_PATH}/logs" ]; then
	mkdir -p ${TEST_DATANODE_PATH}/${comp_type}
	cp -rf ${TEST_DATANODE_PATH}/data ${TEST_DATANODE_PATH}/${comp_type}
	cp -rf ${TEST_DATANODE_PATH}/conf ${TEST_DATANODE_PATH}/${comp_type}
	mv ${TEST_DATANODE_PATH}/logs ${TEST_DATANODE_PATH}/${comp_type}	
	fi
	#同步服务器监控数据到统一的表内
	#drop_monitor_table
	###############################unseq_space合并###############################
	...
	sed -i "s/^#MAX_HEAP_SIZE=\"2G\".*$/MAX_HEAP_SIZE=\"20G\"/g" ${TEST_DATANODE_PATH}/conf/datanode-env.sh
	#关闭影响写入性能的其他功能
	sed -i "s/^enable_seq_space_compaction=.*$/enable_seq_space_compaction=false/g" ${TEST_DATANODE_PATH}/conf/iotdb-common.properties

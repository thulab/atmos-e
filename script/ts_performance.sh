...
monitor_test_status() { # 监控测试运行状态，获取最大打开文件数量和最大线程数
	...
	do
		...
		ts_status1=$(cat ${TEST_IOTDB_PATH}/tools/log.txt | grep 'successfully'| wc -l)
		ts_status2=$(cat ${TEST_IOTDB_PATH}/tools/log.txt | grep 'completely'| wc -l)
...
test_operation() {
	...
	sleep 30
	start_time=`date`
	ts_state=$(${TEST_IOTDB_PATH}/tools/load-tsfile.sh -f ${DATA_PATH}/${data_type}/${ts_type} -h 127.0.0.1 -p 6667 -u root -pw root -os none -of none >${TEST_IOTDB_PATH}/tools/log.txt &)
	monitor_test_status
	#cost_time=$(($(date +%s -d "${end_time}") - $(date +%s -d "${start_time}")))
	...
	sed -i "s/^# max_deduplicated_path_num=1000.*$/max_deduplicated_path_num=600000/g" ${TEST_IOTDB_PATH}/conf/iotdb-system.properties
	sed -i "s/^# query_timeout_threshold=60000.*$/query_timeout_threshold=600000/g" ${TEST_IOTDB_PATH}/conf/iotdb-system.properties
	#启动iotdb
	start_iotdb
	sleep 10	
	####判断IoTDB是否正常启动

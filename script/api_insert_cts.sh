#!/usr/bin/env bash
# ------------------------------------------------------------------------------
# IoTDB API Insert CTS Script
# ------------------------------------------------------------------------------
# Description: 自动化执行 IoTDB API CTS 写入测试，并将结果回写到 MySQL。
# Notes:
#   1. 该脚本实际依赖 Bash 语法，因此显式使用 bash 解释器。
#   2. 本次重构优先处理状态流转、结果解析、路径保护和可维护性问题。
# ------------------------------------------------------------------------------
...
readonly BACKUP_PATH="/nasdata/repository/api_insert_cts"
...
readonly -a TS_LIST=(SESSION_BY_TABLET SESSION_BY_RECORDS SESSION_BY_RECORD JDBC)
...
readonly TABLENAME="test_result_api_insert_cts"
readonly TABLENAME_T="test_result_api_insert_cts"

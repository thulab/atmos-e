...
modify_iotdb_config() { # iotdb调整内存，关闭合并
	#修改IoTDB的配置
	sed -i "s/^@REM set ON_HEAP_MEMORY=2G.*$/set ON_HEAP_MEMORY=2G/g" ${TEST_IOTDB_PATH}/conf/datanode-env.bat
	sed -i "s/^@REM set ON_HEAP_MEMORY=2G.*$/set ON_HEAP_MEMORY=500M/g" ${TEST_IOTDB_PATH}/conf/confignode-env.bat
	#调整测试参数
	sed -i "s/^# data_region_group_extension_policy=.*$/data_region_group_extension_policy=CUSTOM/g" ${TEST_IOTDB_PATH}/conf/iotdb-common.properties
	sed -i "s/^# default_data_region_group_num_per_database=.*$/default_data_region_group_num_per_database=1/g" ${TEST_IOTDB_PATH}/conf/iotdb-common.properties
	sed -i "s/^# datanode_memory_proportion=.*$/datanode_memory_proportion=11:6:1:0:0:2/g" ${TEST_IOTDB_PATH}/conf/iotdb-common.properties
	sed -i "s/^# compaction_write_throughput_mb_per_sec=.*$/compaction_write_throughput_mb_per_sec=4/g" ${TEST_IOTDB_PATH}/conf/iotdb-common.properties
	sed -i "s/^# compaction_read_throughput_mb_per_sec=.*$/compaction_read_throughput_mb_per_sec=10/g" ${TEST_IOTDB_PATH}/conf/iotdb-common.properties
	sed -i "s/^# enable_last_cache=.*$/enable_last_cache=false/g" ${TEST_IOTDB_PATH}/conf/iotdb-common.properties
	#关闭影响写入性能的其他功能
...
collect_monitor_data() { # 收集iotdb数据大小，顺、乱序文件数量
	...
	maxDiskIOOpsRead=$(get_single_index "max_over_time(disk_io_ops{instance=~\"${IoTDB_IP}:9091\",disk_id=~\"vdc\",type=~\"read\"}[$((m_end_time-m_start_time))s])" $m_end_time)
	maxDiskIOOpsWrite=$(get_single_index "max_over_time(disk_io_ops{instance=~\"${IoTDB_IP}:9091\",disk_id=~\"vdc\",type=~\"write\"}[$((m_end_time-m_start_time))s])" $m_end_time)
	maxDiskIOSizeRead=$(get_single_index "max_over_time(disk_io_size{instance=~\"${IoTDB_IP}:9091\",disk_id=~\"vdc\",type=~\"read\"}[$((m_end_time-m_start_time))s])" $m_end_time)
	maxDiskIOSizeWrite=$(get_single_index "max_over_time(disk_io_size{instance=~\"${IoTDB_IP}:9091\",disk_id=~\"vdc\",type=~\"write\"}[$((m_end_time-m_start_time))s])" $m_end_time)
}

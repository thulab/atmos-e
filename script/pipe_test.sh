...
setup_env() {
	...
	do
		TEST_IP=${IP_list[$j]}
		ssh ${ACCOUNT}@${IP_list[${j}]} "ps -ef | grep java | grep -v grep | grep '^root' | awk '{print $2}' | xargs kill -9 > /dev/null 2>&1 &"
		ssh ${ACCOUNT}@${IP_list[${j}]} "sudo sync"
		ssh ${ACCOUNT}@${IP_list[${j}]} "sudo echo 3 > /proc/sys/vm/drop_caches"
	done
	sleep 120
	for (( i = 1; i < ${#IP_list[*]}; i++ ))
	do

...
#C_IP_list=(0 172.20.70.6 172.20.70.10 172.20.70.12)
...
modify_iotdb_config() { # iotdb调整内存，关闭合并
	...
	sed -i "s/^#MAX_HEAP_SIZE=\"2G\".*$/MAX_HEAP_SIZE=\"6G\"/g" ${TEST_CONFIGNODE_PATH}/conf/datanode-env.sh
	#关闭影响写入性能的其他功能
...
if [ "$check_config_num" == "$config_num" ] && [ "$check_data_num" == "$data_num" ]; then
	...
	then
		for ((j = 1; j <= $bm_num; j++)); do
			...
			scp -rf ${BM_PATH}/conf/config.properties ${ACCOUNT}@${B_IP_list[${j}]}:${BM_PATH}/conf/config.properties
	...
	while true; do
		...
		do
			...
			if [ "${iotdb_pid}" = "" ]; then
				...
				temp_file_num_c=$(ssh ${ACCOUNT}@${C_IP_list[${j}]} "ps aux | grep -v grep | grep DataNode | awk '{print \$2}' | xargs /usr/sbin/lsof -p | wc -l" 2>/dev/null)
			fi
			iotdb_pid=$(jps | grep ConfigNode | awk '{print $1}')
			if [ "${iotdb_pid}" = "" ]; then
				echo "无ConfigNode程序！"
				...
				temp_thread_num_c=$(ssh ${ACCOUNT}@${C_IP_list[${j}]} "pstree -p \$(ps -e | grep ConfigNode | awk '{print \$1}') | wc -l" 2>/dev/null)
			fi				
			#监控打开文件数量			

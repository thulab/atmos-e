...
modify_iotdb_config() { # iotdb调整内存，关闭合并
	...
	#修改集群名称
	sed -i "s/^cluster_name=.*$/cluster_name=IoTDB-Enterprise-20/g" ${TEST_DATANODE_PATH}/conf/iotdb-system.properties
	sed -i "s/^cluster_name=.*$/cluster_name=IoTDB-Enterprise-20/g" ${TEST_CONFIGNODE_PATH}/conf/iotdb-system.properties
	#添加启动监控功能
	...
	#添加多路径
	#sed -i "s%^# dn_data_dirs=data/datanode/data.*$%dn_data_dirs=data/datanode/data,/data2/datanode/data%g" ${TEST_DATANODE_PATH}/conf/iotdb-system.properties
}
set_protocol_class() { 
	config_node=$1
	schema_region=$2

...
do
	...
	if [ "${commit_id}" = "" ]; then
		...
		#清理过期文件 - 当前策略保留4天
		find ${BUCKUP_PATH}/unseq_w -mtime +4 -type d -name "*" -exec rm -rf {} \;
		find ${BUCKUP_PATH}/unseq_rw -mtime +4 -type d -name "*" -exec rm -rf {} \;
		find ${BUCKUP_PATH}/seq_w -mtime +4 -type d -name "*" -exec rm -rf {} \;
		find ${BUCKUP_PATH}/seq_rw -mtime +4 -type d -name "*" -exec rm -rf {} \;
	fi
done

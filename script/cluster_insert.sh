...
modify_iotdb_config() { # iotdb调整内存，关闭合并
	...
	#修改集群名称
	echo "cluster_name=IoTDB-Enterprise" >> ${TEST_CONFIGNODE_PATH}/conf/iotdb-system.properties
	echo "cluster_name=IoTDB-Enterprise" >> ${TEST_DATANODE_PATH}/conf/iotdb-system.properties
	#添加启动监控功能
...
do
	...
	ssh ${ACCOUNT}@${IP_list[${i}]} "sudo systemctl reboot"
done
sleep 180
for (( i = 1; i < ${#IP_list[*]}; i++ ))
do

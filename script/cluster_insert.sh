...
modify_iotdb_config() { # iotdb调整内存，关闭合并
	...
	#修改集群名称
	sed -i "s/^cluster_name=.*$/cluster_name=IoTDB-Enterprise/g" ${TEST_DATANODE_PATH}/conf/iotdb-system.properties
	sed -i "s/^cluster_name=.*$/cluster_name=IoTDB-Enterprise/g" ${TEST_CONFIGNODE_PATH}/conf/iotdb-system.properties
	#添加启动监控功能

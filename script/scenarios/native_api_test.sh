#!/bin/bash
# 场景名称：Native API 测试
# 测试目的：验证 TimechoDB/IoTDB 原生 Python API 测试用例的执行结果和成功率。
# 具体步骤：
# 步骤一：准备待测 TimechoDB 包和 Python native API 用例仓库。
# 步骤二：部署 TimechoDB，拷贝 license 和环境配置并启动服务。
# 步骤三：创建 Python 虚拟环境并安装测试依赖。
# 步骤四：执行 pytest 原生 API 用例并生成测试报告。
# 步骤五：解析通过率和耗时，回写数据库并备份报告。
#登录用户名
ACCOUNT=root
...
EOF
			...
			echo "备份Python原生接口测试报告"
      		mkdir -p /data/qa/backup/python/${commit_id_iotdb}_${failures_num}
      		cp -rf  ${TEST_PYTHON_TOOL_PATH}/reports/* /data/qa/backup/python/${commit_id_iotdb}_${failures_num}
			return 1
		fi
	else
		#收集测试结果

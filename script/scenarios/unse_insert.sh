#!/usr/bin/env bash
# 场景名称：乱序数据写入测试
# 测试目的：验证乱序时序数据在不同协议、数据模型和写入模式下的写入性能。
# 具体步骤：
# 步骤一：选择待测 IoTDB 版本并同步 benchmark 工具。
# 步骤二：部署 IoTDB，应用乱序写入场景配置并启动服务。
# 步骤三：启动 benchmark 执行乱序数据写入。
...
readonly SCENARIO_ID="unse_insert"
readonly TASK_TEST_TYPE="unse_insert"

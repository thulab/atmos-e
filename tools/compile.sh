		...
		if [ "${abc}" = "" ]; then
			...
			then
				...
				if [ "${file_num}" = "0" ]; then
					#不需要测试
					str_noneed='NoNeed'
					insert_sql="insert into ${TABLENAME} (commit_date_time,commit_id,author,se_insert,unse_insert,se_query,unse_query,compaction,iotdb_sql,weeklytest_insert,weeklytest_query,api_test,routine_test,config_insert,ts_performance) values(${commit_date_time},'${commit_id}','${author}','${str_noneed}','${str_noneed}','${str_noneed}','${str_noneed}','${str_noneed}','${str_noneed}','${str_noneed}','${str_noneed}','${str_noneed}','${str_noneed}','${str_noneed}','${str_noneed}')"
